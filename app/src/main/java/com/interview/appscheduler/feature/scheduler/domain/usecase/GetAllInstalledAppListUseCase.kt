...
class GetAllInstalledAppListUseCase @Inject constructor(
    private val repository: AbstractAppSchedulerRepository
) : AbstractUseCase<Flow<Result<Entity<List<AppEntity>>>>, Unit> {
    override suspend operator fun invoke(): Flow<Result<Entity<List<AppEntity>>>> =  repository.getInstalledAppList()
